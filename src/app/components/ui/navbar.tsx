...
export default function Navbar() {
  ...
  return (
    <div className='w-full h-20 bg-neutral-900 bg-opacity-60 z-20 flex justify-around  items-center fixed top-0 hover:border-b hover:border-stone-900 '>
        <div >
            <h3 className='font-medium text-2xl'>Musyafa Achmad();</h3>
        </div>
        <div className=''>
            <ul className='flex justify-evenly gap-7'>
                {dataMenu.map((item,index) => (
                 <li key={index} className='text-sm '>
                    <Link href={item.link}>
                    <span  className={`hover:text-slate-200 px-3 py-3 rounded ${pathname === item.link ?  'bg-black bg-opacity-20 text-white': ''}`} > 
                        {item.title}
                    </span>
                    </Link>
                </li>
